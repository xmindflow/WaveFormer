...
class FET(nn.Module):
    def __init__(self, dim, num_heads, sr_ratio, bridge=False, L_num=3):
        ...
        self.boundary_lvl_agg = nn.Conv3d(dim//4, dim//4, kernel_size=(self.L_num, 1, 1), bias=False, groups=dim//4)
        self.sig = nn.Sigmoid()
    ...
    def forward(self, x, H, W, q=None):
        ...
        global_context = global_context.softmax(dim=-1)
        ...
        out_spatial_boundary = self.layer_norm2(out_spatial_boundary)
        out_efficient_att =    self.layer_norm3(out_efficient_att)
        comb = self.layer_norm(torch.cat([out_spatial_boundary, out_efficient_att], dim=-1))
        
        # Final Projection
        out = self.proj(comb)
#         out = out_spatial_boundary
...
class QueryGenerator(nn.Module):
    ...
    def forward(self, x):
        ...
        q_lvl2 = x[..., 3136:5096].reshape(B, -1, int(C*2.5))
        q_lvl3 = x[..., 5096:5880].reshape(B, -1, C*4)

        return [q_lvl1, q_lvl2, q_lvl3]


class BridgeLayer(nn.Module):
    def __init__(self, dims, num_heads=[4, 10, 16], sr_ratio=[1, 1, 1], L_num=3):
        super().__init__()
        ...
        self.wave_lvl_2 = FET(int(dims*2.5), num_heads[1], sr_ratio[1], bridge=True, L_num=L_num)
        self.wave_lvl_3 = FET(    dims*4   , num_heads[2], sr_ratio[2], bridge=True, L_num=L_num)

        
        # MixFFN
        self.mixffn1 = MixFFN_skip(    dims     , dims*4)
        self.mixffn2 = MixFFN_skip(int(dims*2.5), dims*10)
    ...
    def forward(self, inputs):
        ...
        inputs = Rearrange('b (h w) c -> b c h w', h=120, w=49)(inputs)
        ...
        lvl2 = tx[:,3136:5096,:].reshape(B, -1, int(C*2.5))
        lvl3 = tx[:,5096:5880,:].reshape(B, -1, C*4)
        
        q_lvl1, q_lvl2, q_lvl3 = self.queries(tx1.permute(0, 2, 1))
...
class BridegeBlock(nn.Module):
    def __init__(self, dims, num_heads=[4, 10, 16], sr_ratio=[1, 1, 1], bridge_layers=2, L_num=3):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        sk2 = x[:,3136:5096,:].reshape(B, 28, 28, int(C*2.5)).permute(0,3,1,2) 
        sk3 = x[:,5096:5880,:].reshape(B, 14, 14, C*4).permute(0,3,1,2) 
...
class Model(nn.Module):
    def __init__(self, num_classes=9, num_heads_enc=[4, 10, 16], sr_ratio=[1, 1, 1], num_heads_dec= [8, 4, 1],
                 bridge_layers=2, token_mlp_mode="mix_skip", encoder_L_num=3, bridge_L_num=3, decoder_L_num=3):
        super().__init__()
        ...
        self.skip_connection = BridegeBlock(dims=dims[0], num_heads=num_heads_enc, sr_ratio=[1, 1, 1],
                                            bridge_layers=bridge_layers, L_num=bridge_L_num)
        
        # Decoder
        d_base_feat_size = 7 #16 for 512 input size, and 7 for 224
        in_out_head_sr = [[72, 128, num_heads_dec[0], sr_ratio[-1]], [288, 320, num_heads_dec[1], sr_ratio[-2]],
                          [512, 512, num_heads_dec[2], sr_ratio[-3]]]

        self.decoder_3 = MyDecoderLayer((d_base_feat_size*2, d_base_feat_size*2), in_out_head_sr[2], 
                                        token_mlp_mode, n_class=num_classes, L_num=decoder_L_num)
        self.decoder_2 = MyDecoderLayer((d_base_feat_size*4, d_base_feat_size*4), in_out_head_sr[1],
                                        token_mlp_mode, n_class=num_classes, L_num=decoder_L_num)
        self.decoder_1 = MyDecoderLayer((d_base_feat_size*8, d_base_feat_size*8), in_out_head_sr[0], 
    ...
    def forward(self, x):
        ...
        #---------------Decoder-------------------------     
        tmp_3 = self.decoder_3(output_enc[2].permute(0,2,3,1).view(b,-1,c))
        tmp_2 = self.decoder_2(tmp_3, output_enc[1].permute(0,2,3,1))
